...
if (!$user_data['userid']) $func->error($lang['usrmgr']['checkin_nouser'], '');
  ...
  if ($db->num_rows($user_fields) > 0) $menunames[4] = $lang['usrmgr']['details_own_fields'];
	...
	switch($vars['headermenuitem']){
      ...
      // Picture
            if (func::chk_img_path($user_data['picture'])) $dsp->AddDoubleRow($lang['usrmgr']['picture'], '<img src="'. $user_data['picture'] .'">');
		...
		case 2:
		    if (in_array('tournament2', $ActiveModules)) { 
                ...
                $dsp->AddFieldsetStart(t('Benutzer hat folgende Teams er&ouml;ffnet'));
                    ...
                    if ($db->num_rows($leader_teams) == 0) $dsp->AddSingleRow('<i>-'. t('Keine') .'-</i>');
                ...
                $dsp->AddFieldsetStart(t('Benutzer ist in folgenden Teams Mitglied'));
                    ...
                    if ($db->num_rows($member_teams) == 0) $dsp->AddSingleRow('<i>-'. t('Keine') .'-</i>');
