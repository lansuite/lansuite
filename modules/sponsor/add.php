...
if ($_GET['action'] == 'change' and $_GET['sponsorid'] == '') {
    ...
    $mf->AddField(t('Ziel-URL'), 'url', '', '', cFIELD_OPTIONAL);
    $mf->AddGroup('General');
    ...
    $mf->AddField(t('Auf Sponsorenseite').'|'.t('Der Banner wird auf der Sponsorenseite angezeigt'), 'sponsor', 'tinyint(1)', '', masterform::FIELD_OPTIONAL, '', 3);
    $mf->AddField(t('Bild-Upload'), 'pic_upload', masterform::IS_FILE_UPLOAD, 'ext_inc/banner/', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Oder: Bild-URL'), 'pic_path', 'varchar(255)', '', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Oder: Bild-Code (z.B. Flash)') . $code_popup_link_banner, 'pic_code', 'text', '', masterform::FIELD_OPTIONAL);
    $mf->AddGroup('Sponsorenseite');

    $mf->AddField('', '', masterform::IS_TEXT_MESSAGE, t('Wenn du hier keine Datei angibst, wird der Banner/Button automatisch durch verkleinern der oben angegebenen Datei erzeugt (Funktioniert nur bei heraufgeladenen Dateien).'));
    $mf->AddGroup('');

    $mf->AddField(t('In Rotations-Banner').'|'.t('Der Banner wird oben in den Rotations-Banner aufgenommen'), 'rotation', 'tinyint(1)', '', masterform::FIELD_OPTIONAL, '', 4);
    $mf->AddField(t('Bild-Upload'), 'pic_upload_banner', masterform::IS_FILE_UPLOAD, 'ext_inc/banner/', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Oder: Bild-URL'), 'pic_path_banner', 'varchar(255)', '', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Oder: Bild-Code (z.B. Flash)') . $code_popup_link_banner, 'pic_code_banner', 'text', '', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Bei HTTPS verstecken').'|'.t('Diese Option solltest du aktivieren, wenn du den Banner-Code von einer externen Webseite lädst und er dort nur als HTTP Version verfügbar ist, du deine Webseite aber als HTTPS ausliefern möchtest.'), 'ssl_hide_banner', '', '', masterform::FIELD_OPTIONAL);
    $mf->AddGroup('Rotation Banner');

    $mf->AddField(t('In Sponsoren-Box').'|'.t('Der Banner wird in der Sponsoren-Box angezeigt'), 'active', 'tinyint(1)', '', masterform::FIELD_OPTIONAL, '', 4);
    $mf->AddField(t('Bild-Upload'), 'pic_upload_button', masterform::IS_FILE_UPLOAD, 'ext_inc/banner/', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Oder: Bild-URL'), 'pic_path_button', 'varchar(255)', '', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Oder: Bild-Code (z.B. Flash)') . $code_popup_link_box, 'pic_code_button', 'text', '', masterform::FIELD_OPTIONAL);
    $mf->AddField(t('Bei HTTPS verstecken').'|'.t('Diese Option solltest du aktivieren, wenn du den Banner-Code von einer externen Webseite lädst und er dort nur als HTTP Version verfügbar ist, du deine Webseite aber als HTTPS ausliefern möchtest.'), 'ssl_hide_button', '', '', masterform::FIELD_OPTIONAL);
    ...
    $mf->AddField(t('Text'), 'text', '', masterform::HTML_ALLOWED, masterform::FIELD_OPTIONAL);
    $mf->AddGroup('Misc.');
