...
// Check Generated
if ($t['status'] != "process" and $t['status'] != "closed") $func->information(t('Dieses Turnier wurde noch nicht generiert. Die Paarungen sind noch nicht bekannt.'), 'index.php?mod=tournament2&action=tree&step=1');

// Check Teams
elseif ($team_anz == 0) $func->information(t('Es sind keine Teams zu diesem Turnier angemeldet'), 'index.php?mod=tournament2&action=tree&step=1');
  ...
  if ($cfg['t_text_tree']) {
    ...
    ';
  	...
  	function write_pairs2 ($bracket, $max_pos) {
       ...
       ", $tournamentid, $akt_round, $akt_pos);
  			...
  			if ($spieler1 == "") {
  				...
  				$link = '';
  				if ($gameid1 and $gameid2) $link = "index.php?mod=tournament2&action=submit_result&step=1&tournamentid=$tournamentid&gameid1=$gameid1&gameid2=$gameid2";
  		    ...
  		    $templ['index']['info']['content'] .= "CreateLine(". ($xpos + $box_width) .", ". ($ypos + $box_height / 2) .", ". ($xpos + $box_width + 5) .", ". ($ypos + $box_height / 2) .", '#000000');";
      		    ...
      		    $templ['index']['info']['content'] .= "CreateLine(". ($xpos + $box_width + 4) .", ". ($line_start + $box_height / 2) .", ". ($xpos + $box_width + 4) .", ". ($ypos + $box_height / 2) .", '#000000');";
  					...
  					$line_start = 0;
  				} else $line_start = $ypos;
  
  				// Write Player1
  				if (!$known_game1) $t_color = $color["unknown"];
  				elseif ($spielerid1 == 0) $t_color = $color["freilos"];
  				elseif ($score1 > $score2 and $known_game2) $t_color = $color["winner"];
  				elseif ($score1 < $score2 and $known_game2) $t_color = $color["loser"];
  		    ...
  		    $templ['index']['info']['content'] .= "CreateText('vs', ". ($xpos + ($box_width / 2) - 10) .", ". ($ypos +23) .", '$link');";
  				...
  				// Write Player2
  				if (!$known_game2) $t_color = $color["unknown"];
  				elseif ($spielerid2 == 0) $t_color = $color["freilos"];
  				elseif ($score1 < $score2 and $known_game1) $t_color = $color["winner"];
  	...
  	if ($t['mode'] == "single" or $t['mode'] == "double" or ($t["mode"] == "groups" and $_GET["group"] == 0)) {
  		...
  		$akt_round = 1;
  		if ($t['mode'] == "double") $limit_round = 2;
  		else $limit_round = 4;
     ...
     ", $tournamentid, $_GET["group"]);
       ...
       ", $tournamentid, $_GET["group"], $leader_array[$x], $leader_array[$y-1], $leader_array[$y-1], $leader_array[$x]);
  				...
  				else  $game_score = $score['s1']. " : " .$score['s2'];
  
    			$link = '';
    			if ($score['gameid1'] and $score['gameid2']) $link = "index.php?mod=tournament2&action=submit_result&step=1&tournamentid=$tournamentid&gameid1={$score['gameid1']}&gameid2={$score['gameid2']}";
