...
class TourneyTree {
    ...
    var $wb_num_cols;
    var $wb_tbl = array();
    var $wb_indexes = array();
    var $lb_rounds;
    var $lb_num_rows;
    var $lb_num_cols;
    var $lb_tbl = array();
    var $lb_indexes = array();

    function TourneyTree($size, $wb_teams, $lb_teams=false) {
        // init vars !
        $this->size = $size;
    ...
    function numWBRounds($size) {
    	return TourneyTree::log2($size);
    }

    function numLBRounds($size) {
    ...
    function getPrevCoords($round, $indexes, $offset=2) {
        $tmp = $indexes;
        $tmp2 = $round-$offset;
    ...
    function generateBar($x, $y, &$tbl) {
        $tbl[$x][$y] = true;
    }

    function generateArrows(&$tbl) {
        for($i=1; $i < count($tbl);) {
            $t = $s = 0;
            for ($x=0; $x < count($tbl[$i]); $x++) {
                if ($tbl[$i][$x] === true && $t == 0) $s=$x;
                if ($tbl[$i][$x] === true) $t++;
    ...
    function LBTeamsInRound($round, $size) {
        $i = (2*TourneyTree::log2($size))-1;
        if ($round < 1 || $round >= $i) return false;
    ...
    function WBTeamsInRound($round, $size) {
        $max_rounds = TourneyTree::log2($size);
        if ($round < 1 || $round > $max_rounds) return false;
        return pow(2, ($max_rounds - ($round - 1)));
    }

    function generateFakeTeams(&$wb, &$lb) {
        $wb = $lb = array();
                    ...
                    if ($x > 0 && $x < ($this->lb_num_cols-2) && ($x % 2) == 0) {
                        ...
                        if ($this->calcMW($tmp[0][1], $tmp[1][1]) == $y) {
                            if (count($this->lb_teams[($x/2)]) > 0)
                                $this->lb_tbl[$x][$y] = array_shift($this->lb_teams[($x/2)]);        // insert team
