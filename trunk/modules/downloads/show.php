...
// Try to connect to FTP-Server
  ...
  if ($connect != FALSE AND $login != FALSE) {
    if ($_GET[go_dir] == "up") array_pop($_SESSION['downloads_dir']);  
    elseif ($_GET[go_dir]) {
      if(count($_SESSION['downloads_dir']) > "0") foreach($_SESSION['downloads_dir'] AS $dir_entry) $set_dir .= "/" . $dir_entry; 
      
      $join_dir = @ftp_chdir($connect, $subdir . $set_dir . "/" . $_GET[go_dir]);
      if($join_dir == TRUE AND $_GET[go_dir] != "." AND $_GET[go_dir] != "..") $_SESSION['downloads_dir'][] = $_GET[go_dir]; 
    ...
    if (count($_SESSION['downloads_dir']) > "0") foreach($_SESSION['downloads_dir'] AS $dir_entry) $set_dir .= "/" . $dir_entry;
    if ($subdir != "" OR $set_dir != "") $join_dir = @ftp_chdir($connect, $subdir . $set_dir);
    ...
    if ($content) {
      ...
      foreach ($content AS $cur_line) {
        ...
        if(ereg("([-d])([rwxst-]{9}).* ([0-9]*) ([a-zA-Z]+[0-9: ]* [0-9]{2}:?[0-9]{2}) (.+)", $cur_line, $regs)) {      
          ...
          else $lineinfo['folder'] = FALSE;
          $lineinfo['size']   = $regs[3];
          $lineinfo['name']   = $regs[5];
          if($lineinfo['size'] < "1024") $lineinfo['rounded_size'] = "< 1 KB";
          elseif($lineinfo['size'] >= "1024000") $lineinfo['rounded_size'] = round($lineinfo['size'] / "1024" / "1000","2") . " MB";
          else $lineinfo['rounded_size'] = round($lineinfo['size'] / "1024" ,"2") . " KB";

          $lineinfo['type']   = explode(".",$lineinfo['name']);
          if(count($lineinfo['type']) > "1") $lineinfo['datatype'] = $lineinfo['type'][count($lineinfo['type'])-1] . "-Datei";
        }
    
        if ($lineinfo['size'] > "0" AND $lineinfo['folder'] != TRUE) {
          $loginpassword = str_replace("@", "(at)", $loginpassword);

          if($loginuser == "" AND $loginpassword == "") $account = "";
          else $account = $loginuser . ":" . $loginpassword;
