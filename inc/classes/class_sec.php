...
class sec
{
    public function check_blacklist()
    {
        ...
        if (strpos($cfg['ip_blacklist'], $_SERVER['REMOTE_ADDR']) !== false) {
            die("Deine IP wird von LanSuite geblockt. Melde dich bitte bei den Administratoren");
        }
        ...
        if (!$cfg["reload_time"]) {
            ...
            if ($ip_hits["hits"] > $cfg["reload_hits"]) {
                die("Deine IP wird von LanSuite wegen zu häufigen Seitenaufrufen geblockt. Bitte warte ein wenig und versuche es dann erneut.");
            }
        }
    }
