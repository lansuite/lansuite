...
if ($team_anz != 0 and ($tournament['status'] == "process" or $tournament['status'] == "closed")) {
	...
	function write_pairs ($bracket, $max_pos) {
		...
		for ($akt_pos = 0; $akt_pos <= $max_pos-1; $akt_pos++) {
			...
			if ($spieler1 == "") {
				...
				if ($line_start) {
					if ($akt_round <= 0){
						if ($akt_round == floor($akt_round)) ImageLine($gd->img, $xpos - 5, $line_start + $box_height / 2, $xpos - 5, $ypos + $box_height / 2, $color["frame"]);
					}
					if ($akt_round >= 0){
	...
	if (($tournament['mode'] == "liga")
		...
		for ($y = 1; $y <= $team_anz; $y++){
			...
			for ($x = 0; $x < $y-1; $x++){
				$score = $db->query_first("SELECT games1.score AS s1, games2.score AS s2, games1.leaderid AS leader1
