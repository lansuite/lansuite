    ...
    if ($_GET["action"] == "wizard" and $_GET['step'] < 9) {
      ...
      $file = "modules/$module/mod_settings/module.xml";
              ...
              if (file_exists($file)) {
                  ...
                  if ($name) {
                      if (!$mod_found["found"]) {
                          $db->qry_first("REPLACE INTO %prefix%modules
                              ...
                              if ($xml_entries) {
                                  while ($xml_entry = array_shift($xml_entries)) {
                                      ...
                                      if ($name != '' and $description != '') {
                                          $db->qry("INSERT INTO %prefix%config_selections SET cfg_key = %string%, cfg_value = %string%, cfg_display = %string%",
      $name, $value, $description);
          ...
          $xml_groups = $xml->get_tag_content_array('group', $xml_config);
              ...
              $found = $db->qry_first("SELECT cfg_key FROM %prefix%config WHERE cfg_key = %string%", $name);
                                      if (!$found['cfg_key']) {
                                          $db->qry("INSERT INTO %prefix%config SET cfg_key = %string%, cfg_value = %string%, cfg_type = %string%, cfg_group = %string%, cfg_desc = %string%, cfg_module = %string%, cfg_pos = %int%",
