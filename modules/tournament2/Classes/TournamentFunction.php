...
class TournamentFunction
{
    ...
    public function GetTeamAnz($tid, $mode, $group = 0)
    {
        ...
        ('liga' === $mode || 'groups' === $mode) ? $add_where = 'AND (leaderid != 0)' : $add_where = '';
    ...
    public function GetGameStart($tournament, $round, $group_nr = 0)
    {
        ...
        ('double' === $tournament['mode']) ? $faktor = 2 : $faktor = 1;

        // If final games of a group-tournament add time for group-games
        if (('groups' === $tournament['mode']) && (0 === $group_nr)) {
        ...
        while ($row = $db->fetch_array($res)) {
            if ($row['start'] > $tournament['starttime'] && $row['start'] < $time) {
    ...
    public function get_ranking($tournamentid, $group_nr = null)
    {
     ...
     ', $tournamentid, $tournamentid);
                ...
                // Array schreiben
                while ($team = $db->fetch_array($teams)) {
                    if ($team['teamid'] && !\in_array($team['teamid'], $ranking_data->tid, true)) {
                        ++$array_id;
                        array_push($ranking_data->id, $array_id);
                        array_push($ranking_data->tid, $team['teamid']);
                        array_push($ranking_data->name, $team['name']);
     ...
     ', $tournamentid);
            ...
            while ($score = $db->fetch_array($scores)) {
                if ('groups' === $tournament['mode'] && 0 === $group_nr) {
                    $ranking_data->reached_finales[array_search($score['tid1'], $ranking_data->tid, true)] = 1;
                    $ranking_data->reached_finales[array_search($score['tid2'], $ranking_data->tid, true)] = 1;
                }
                $ranking_data->score[array_search($score['tid1'], $ranking_data->tid, true)] += $score['s1'];
                $ranking_data->score[array_search($score['tid2'], $ranking_data->tid, true)] += $score['s2'];
                $ranking_data->score_en[array_search($score['tid1'], $ranking_data->tid, true)] += $score['s2'];
                $ranking_data->score_en[array_search($score['tid2'], $ranking_data->tid, true)] += $score['s1'];

                ++$ranking_data->games[array_search($score['tid1'], $ranking_data->tid, true)];
                ++$ranking_data->games[array_search($score['tid2'], $ranking_data->tid, true)];

                if ($score['s1'] === $score['s2']) {
                    ++$ranking_data->win[array_search($score['tid1'], $ranking_data->tid, true)];
                    ++$ranking_data->win[array_search($score['tid2'], $ranking_data->tid, true)];
    ...
    public function SubmitResult($ttid, $gameid1, $gameid2, $score1, $score2, $comment)
    {
        ...
        // KO-Systems
        if (('single' === $tournament['mode']) || ('double' === $tournament['mode'])
            || (('groups' === $tournament['mode']) && (0 === $gr_game['group_nr']))) {
            $num_rounds = 1;
            ...
            if (('single' === $tournament['mode']) && $round === ($num_rounds - 1)) {
                $unfinished_games = $db->qry_first('
                  SELECT
                    games1.gameid
                  FROM %prefix%t2_games AS games1
            ...
            if (($round === $num_rounds)
              || (('groups' === $tournament['mode']) && ($round === $num_rounds - 1))
              || (('single' === $tournament['mode']) && ($round === $num_rounds - 1) && ('' === $unfinished_games['gameid']))) {
                $db->qry("
    ...
    public function CheckTimeExceed($tournamentid)
    {
        ...
        if ('1' === $tournament['defwin_on_time_exceed']) {
    ...
    private function GenerateNewPosition($player1, $player2)
    {
        ...
        if (($round === floor($round) && $winner) || ($looser && 0 === $round)) {
        ...
        if (($round !== floor($round) && $winner) || ($round === ($num_rounds * (-1) + 1))) {
        ...
        } elseif (($round > 0) && $looser && (floor($round / 2) === $round / 2)) {
        ...
        if ($winner || ($looser && $round >= 0 && $round !== $num_rounds)) {
            $db->qry('
    ...
    private function WriteResult()
    {
        ...
        if ($first) {
            ...
            if ((0 === $score1) && (0 === $score2) && ('' !== $gameid1) && ('' !== $gameid2) && ($leaderid1) && ($leaderid2)) {
                // Choose random winner and set score to default win
                if (0 === $cfg['t_default_win']) {
                    $cfg['t_default_win'] = 2;
                }
                if (1 === rand(0, 1)) {
