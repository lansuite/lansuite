...
class AddUser {
	...
	function GetDBData($action) {
		...
		if ($action == "change"){
			$user_data = $db->query_first("SELECT u.*, s.party_id, p.* FROM {$config["tables"]["user"]} AS u, {$config["tables"]["partys"]} AS s
											LEFT JOIN {$config["tables"]["party_user"]} AS p ON p.party_id=s.party_id AND u.userid=p.user_id
											WHERE u.userid={$_GET["userid"]} AND (p.party_id={$party->party_id} OR ISNULL(p.party_id)) AND s.party_id={$party->party_id}");
	...
	function CheckErrors($action) {
		global $db, $config, $lang, $cfg, $error, $signon, $func, $birthday, $street, $nr, $plz, $city, $auth;
		...
		if (preg_match("/([.^\"\'`´]+)/",$_POST["username"])) {
			$error["username"] = $lang["usrmgr"]["add_err_user_chars"];
		}
		// Email exist?
		...
		if (($auth["type"] >= 2) && ($_POST["username"] == "")) $error["username"] = $lang["usrmgr"]["add_err_no_user"];
		...
		if ($perso == "<<<<<<<<<<") $perso = "";
		if (($auth["type"] >= 2) && (($perso != "") || ($this->Needed("perso")))){
		...
		if (($auth["type"] >= 2) && ($this->Needed("birthday")) && ($birthday == 0)) $error["birthday"] = $lang["usrmgr"]["add_err_no_birthday"];
	...
	function ShowForm($action) {
		global $db, $config, $lang, $cfg, $dsp, $error, $templ, $birthday, $party, $auth;

		$_SESSION['add_blocker_usrmgr'] = FALSE;

		$dsp->NewContent($lang["usrmgr"]["add_caption"], $lang["usrmgr"]["add_subcaption"]);
		$dsp->SetForm("index.php?mod=usrmgr&action={$_GET["action"]}&umode={$action}&step=". ($_GET["step"] + 1) ."&userid={$_GET["userid"]}", "signon", "", "multipart/form-data");
...
#			$dsp->AddCheckBoxRow("platzpfand", $lang["usrmgr"]["add_platzpfand"], $lang["usrmgr"]["add_platzpfand_detail"], "", 1, $_POST["platzpfand"]);
	...
	function WriteToDB($action) {
		global $db, $config, $lang, $cfg, $signon, $func, $birthday, $street, $nr, $plz, $city, $party, $auth;

		if ($_SESSION['add_blocker_usrmgr']) $func->error("NO_REFRESH", "");
		else {
			$_SESSION['add_blocker_usrmgr'] = TRUE;

			// Passwort generieren / verschüsseln
			if ($action != "change"){
			...
			if ($action == "change"){
				...
				if ($auth["type"] >= 2) {
					$admin_additional = "	username	= '{$_POST["username"]}',
											type		= '{$_POST["type"]}',
											perso		= '$perso',
											birthday	= $birthday,
											comment		= '{$_POST["comment"]}',
										";
				} else $admin_additional = "";
