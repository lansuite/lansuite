...
function DeleteUser($userid) {
  ...
  $get_party_data = $db->qry_first('SELECT 1 AS found FROM %prefix%party_user AS pu
	...
	elseif ($auth["userid"] == $userid) $func->error(t('Sie können sich nicht selbst löschen'), "index.php?mod=usrmgr");
		...
		$db->qry("UPDATE %prefix%user SET type = '-4', username = CONCAT(username,' (Deleted)'), comment = CONCAT('".t('Dieser User wurde gelöscht!')." email: ',email), email = CONCAT(userid,'_deleted') WHERE userid = %int%", $userid);
