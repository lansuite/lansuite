...
$tgames = $db->qry("
  SELECT
    ...
    teams1.leaderid AS leaderid1,
    teams2.leaderid AS leaderid2,
    t.name AS tuname, 
    t.mode AS modus,
    ...
    t.game_duration,
    t.break_duration,
    IF (games1.round = 0, (t.game_duration * 60), ((t.game_duration + t.break_duration) * 60)) AS overtime,
    (UNIX_TIMESTAMP(CURRENT_TIMESTAMP()) - UNIX_TIMESTAMP(IF(games1.lastchange > games2.lastchange, games1.lastchange, games2.lastchange))) AS overtime2
...
while ($tgamesrow = $db->fetch_array($tgames, 1, MYSQLI_ASSOC)) {
    if (!($tgamesrow['modus']=="single" and $tgamesrow['round']<0) and !($tgamesrow['modus']=="all")) { // Workaround wegen Looserbraketeinträgen bei SingleElimination
        ...
        $outputrow['Akt.'] ="<div style=\"text-align:right;\">".$dsp->FetchIcon('search', 'index.php?mod=tournament2&action=submit_result&step=1&tournamentid=' . $tgamesrow['tid'] . '&gameid1=' . $tgamesrow['gid1'] . '&gameid2=' . $tgamesrow['gid2'])."</div>";
