...
switch ($stepParameter) {
    ...
    case 2:
        $get_catname = $database->queryWithOnlyFirstRow("SELECT name FROM %prefix%faq_cat WHERE catid = ?", [$_GET['catid']]);
    ...
    case 3:
        ...
        if ($get_catname["name"] != "") {
            $del_cat = $database->query("DELETE FROM %prefix%faq_cat WHERE catid = ?", [$_GET['catid']]);
            $del_item = $database->query("DELETE FROM %prefix%faq_item WHERE catid = ?", [$_GET['catid']]);
