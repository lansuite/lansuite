...
$module = $database->queryWithOnlyFirstRow("SELECT * FROM %prefix%modules WHERE name = 'pdf'");
$pdf = $database->queryWithOnlyFirstRow("SELECT * FROM %prefix%pdf_list WHERE name = ? AND template_type = 'guestcards'", [$cfg['user_ticket_template']]);
