...
class Mastercomment{
	...
	function Mastercomment($vars, $working_link,$modul,$objectid,$objectname) {
      ...
      include("modules/mastercomment/language/mastercomment_lang_$language.php");
		else include("modules/mastercomment/language/mastercomment_lang_de.php");
		$this->lang = $lang;
	}
    ...
    global $auth;
		...
		switch ($this->vars['mcact']){
			default:
				...
				if ($auth['login']) $this->add_comment("add");
				return "show";
			break;	
    ...
    $dsp->AddFieldsetStart(str_replace("%MODULE%", $this->config['modul'], str_replace("%ITEM%", $this->config['objectname'], $this->lang['mastercomment']['comment_caption'])));
		if ($count_entrys > 0){
			...
			while ($row = $db->fetch_array($get_comments)){
				...
				else $templ['mastercomment']['show']['row']['info']['avatar'] = "";
	...
	function add_comment($action){
		...
		switch ($this->vars['mcstep']){
			default:
				...
				$dsp->AddFieldsetStart($this->lang['mastercomment']['comment']);
	...
	function delete_comment(){
		...
		switch($this->vars["mcstep"]) {
			...
			case 2:
  			...
  			if ($delete == 1) $func->confirmation($this->lang['mastercomment']['add_comment_del'],$this->config['working_link']);
