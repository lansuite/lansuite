...
class party{
  ...
  // Constructor
	...
	function set_party_id($id){
        ...
        $dsp->SetForm($link);
				$dsp->AddDropDownFieldRow("party_id",t('Party auswÃ¤hlen')/* TRANS */,$list_array,'');
		...
		function get_price_dropdown($group_id = 0,$price_id = 0,$dropdown = false){
			...
			if($anzahl >1 || $dropdown == true){
				...
				$dsp->AddDropDownFieldRow("price_id",t('Preis auswÃ¤hlen')/* TRANS */,$data,'');
			}else{
				$res = $db->fetch_array($row);
				$dsp->AddDoubleRow(t('Preis auswÃ¤hlen')/* TRANS */,$res['price_text'] . "  / {$res['price']} {$cfg['sys_currency']}<input name='price_id' type='hidden' value='{$res['price_id']}' />");
			}
      ...
      $selections = array();
			...
			$mf->AddField(t('Preis auswÃ¤hlen')/* TRANS */, 'price_id', IS_SELECTION, $selections);
