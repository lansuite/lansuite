...
if ($target_url) $ms2->AddIconField($target_icon, $target_url, t($target_icon));
  ...
  if ($auth['type'] >= 3) $ms2->AddIconField('ip_generate', 'index.php?mod=seating&action=ipgen&blockid=', t('IPs generieren'));
  if ($auth['type'] >= 3) $ms2->AddIconField('ip_edit', 'index.php?mod=seating&action=ip&step=2&blockid=', t('IPs editieren'));
  if ($auth['type'] >= 3) $ms2->AddIconField('ip_del', 'index.php?mod=seating&action=ipgen&step=20&blockid=', t('IPs löschen'));

  if ($auth['type'] >= 2) $ms2->AddIconField('edit', 'index.php?mod=seating&action=edit&step=2&blockid=', t('Editieren'));
  if ($auth['type'] >= 3) $ms2->AddIconField('delete', 'index.php?mod=seating&action=delete&step=2&blockid=', t('Löschen'));
}
