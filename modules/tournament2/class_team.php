...
class team
{
   ...
   ", $userid, $party->party_id);
    ...
    ", $teamid);
      ...
      // Check password, if set and if acction is not performed, by teamadmin or ls-admin
            if (($auth['userid'] != $team['leaderid']) and ($auth['type'] <= 1) and ($team['password'] != '') and !\LanSuite\PasswordHash::verify($password, $team['password'])) {
    ...
    public function create($tournamentid, $leaderid, $name = null, $password = null, $comment = null, $banner = null)
    {
     ...
     banner = %string%,
     password = %string%
     ", $tournamentid, $name, $leaderid, $comment, $_FILES[$banner]["name"], \LanSuite\PasswordHash::hash($password));
   ...
   ", $teamid);
        ...
        // Set Password
        if ($password != "") {
            $db->qry("UPDATE %prefix%t2_teams SET password = %string% WHERE teamid = %int%", \LanSuite\PasswordHash::hash($password), $_GET["teamid"]);
        }
