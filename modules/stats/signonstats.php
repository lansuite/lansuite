...
$get_cur = $db->qry_first("
  ...
  WHERE
    party_id=%int%
    AND (%plain%)", $party->party_id, $querytype);
$cur = $get_cur["n"];

// Wie viele davon haben bezahlt
$get_cur = $db->qry_first("
  SELECT
    COUNT(userid) as n
  FROM %prefix%user AS user
...
$res = $db->qry("
  SELECT
    p.name,
    DATEDIFF(p.startdate, p.sstartdate) AS anmeldetage,
    COUNT(u.user_id) AS angemeldet
  FROM %prefix%party_user AS u
  LEFT JOIN %prefix%partys AS p ON u.party_id = p.party_id
  WHERE
    DATEDIFF(p.startdate, u.signondate) >= %int%
  GROUP BY p.party_id", $party_date['timetoleft']);
