...
class masterform
{
    ...
    {
        if ($type == IS_TEXT_MESSAGE or $type == IS_NOT_CHANGEABLE) {
            $optional = 1;
        }
        $arr = array();
        $arr['caption'] = $caption;
        ...
        if ($this->Pages) {
            ...
            if (!$this->DependOnStarted or $_POST[$this->DependOnField]) {
              ...
              elseif (strpos($SQLFieldTypes[$field['name']], 'int') !== false and $SQLFieldTypes[$field['name']] != 'tinyint(1)'
                ...
                and $_POST[$field['name']] and (int)$_POST[$field['name']] == 0) {
...
#       if (!$this->SQLFields) $func->error('No Fields!');
   ...
   */
  public function get_fieldlenght($string)
  {
