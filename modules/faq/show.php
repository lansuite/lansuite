...
if ($count_cat == 0) {
    ...
    while ($row = $db->fetch_array($get_cat)) {
        if ($auth['type'] > 2) {
            $admin_link = $dsp->FetchIcon('index.php?mod=faq&object=item&action=delete_cat&catid='. $row["catid"] .'&step=2', 'delete');
        }
        if ($auth['type'] > 1) {
            $admin_link .= $dsp->FetchIcon('index.php?mod=faq&object=cat&action=change_cat&catid='. $row["catid"] .'&step=2', 'edit');
        }
        ...
        while ($row = $db->fetch_array($get_item)) {
            if ($auth['type'] > 2) {
                $admin_link = $dsp->FetchIcon('index.php?mod=faq&object=item&action=delete_item&itemid='. $row["itemid"] .'&step=2', 'delete');
            }
            if ($auth['type'] > 1) {
                $admin_link .= $dsp->FetchIcon('index.php?mod=faq&object=cat&action=change_item&itemid='. $row["itemid"] .'&step=2', 'edit');
            }
