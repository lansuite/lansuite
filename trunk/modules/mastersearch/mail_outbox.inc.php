...
// Das SQL Statment ganz nach ihreren Wunsch :)
	$this->config['sql_statment']     = "SELECT mm.mailID AS mailid, 
											...
											ON mm.ToUserID = u.userid";

// WHERE Parameter											
	$this->config['sql_additional'] = " AND mm.FromUserID = '{$_SESSION['auth']['userid']}' AND mail_status != 'disabled'";

// Text der über der Ausgabe steht
//	$this->config['title']            = $lang['ms']['mail_out']['title'];
...
// Wenn nichts gefunden wird dier Text ausgebeben	
	$this->config['no_items_caption'] = $lang['ms']['mail_out']['no_items_caption'];
...
// Spaltenname
	...
	$this->config['result_fields'][$z]['name']     = $lang['ms']['mail_out']['receiver'];
	...
	$z++;
/*
	$this->config['result_fields'][$z]['name']     = $lang['ms']['mail_out']['state'];
	$this->config['result_fields'][$z]['sqlrow']   = "mm.src_status";
	$this->config['result_fields'][$z]['row']      = "src_status";
	$this->config['result_fields'][$z]['callback'] = "ParseOutboxMailStatus";
	$this->config['result_fields'][$z]['width']    = "15%";
	$this->config['result_fields'][$z]['maxchar']  = "0";
	$z++;
*/	
