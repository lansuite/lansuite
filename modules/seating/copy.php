...
switch ($_GET['step']) {
  ...
  case 2:
    ...
    $db->qry('INSERT INTO %prefix%seat_block SET
      ...
      text_bc = %string%,
      text_br = %string%',
      $row['party_id'], $row['rows'], $row['cols'], $row['name'] .' (Kopie)', $row['orientation'], $row['u18'], $row['remark'],
      $row['text_tl'], $row['text_tc'], $row['text_tr'], $row['text_lt'], $row['text_lc'], $row['text_lb'], $row['text_rt'],
      $row['text_rc'], $row['text_rb'], $row['text_bl'], $row['text_bc'], $row['text_br']);
    ...
    while ($row = $db->fetch_array($res)) {
      ...
      $db->qry('INSERT INTO %prefix%seat_seats SET
        ...
        ip = %string%,
        userid = 0',
        $blockid, $row['col'], $row['row'], $row['status'], $row['ip']);
    }
    $db->free_result($res);
    ...
    while ($row = $db->fetch_array($res)) {
        ...
        orientation = %int%,
        value = %int%',
        $blockid, $row['orientation'], $row['value']);
    }
    $db->free_result($res);
