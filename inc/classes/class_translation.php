...
class translation
{
   ...
   * @var mixed
   */
    public $language             = "de";                                 // Global Language
    public $transfile_name       = 'translation.xml';                    // Basename of Translationfile
    public $lang_names           = array('de' => 'Deutsch', 'en' => 'Englisch', 'es' => 'Spanisch', 'fr' => 'Französich', 'nl' => 'Holländisch', 'it' => 'Italienisch');
    public $valid_lang           = array('de','en','es','fr','nl','it'); // Valid Languages
    public $lang_cache           = array();                              // Temporary Translations
    public $cachemod_loaded_db   = 0;                                    // Is Cache for Modul loaded (db)
    public $cachemod_loaded_xml  = 0;                                    // Is Cache for Modul loaded (xml)
  /**#@-*/
   ...
   */
    public function load_cache_bydb($modul)
    {
        global $db;
        if ($db->success) {
            // Load from DB
            ...
            while ($row = $db->fetch_array($res, 0)) {
                if ($row[$this->language] != '') {
                    if ($this->lang_cache[$modul][$row['id']] == '') {
                        $this->lang_cache[$modul][$row['id']] = $row[$this->language];
                    }
                }
            }
        }
   ...
   * @param string Modulname or DB / System
   */
    public function load_cache_byfile($modul)
    {
        // Load from File
   ...
   */
    public function ReplaceParameters($input, $parameters = null, $key = null)
    {
        ...
        if ($key and $auth['type'] >= 2 and $cfg['show_translation_links']) {
   ...
   */
    public function get_calling_module()
    {
        // NOT WORKING
        $out = debug_backtrace();
        return $out;
   ...
   */
    public function get_trans_db($hashkey, $modul, $long)
    {
        global $db;
   ...
   * @return string Translationstring
   */
    public function get_trans_file($hashkey, $modul)
    {
        if ($this->lang_cache[$modul][$hashkey]) {
            $translated = $this->lang_cache[$modul][$hashkey];
        } else {
            if ($this->cachemod_loaded_xml == 0) {
                $this->load_cache_byfile($modul);
            }
            $this->cachemod_loaded_xml = 1;
            if ($this->lang_cache[$modul][$hashkey]) {
                $translated = $this->lang_cache[$modul][$hashkey];
   ...
   */
    public function xml_write_db_to_file($modul)
    {
        global $db;
   ...
   * @access private
   */
    public function xml_var_merge($var1, $var2)
    {
        if ($var2 != "") {
            $out = $var2;
        }
   ...
   */
    public function get_trans_filename($modul)
    {
        switch ($modul) {
            case 'DB':
            ...
            if (substr($file, strlen($file) - 4, 4) == '.php') {
                ...
                foreach ($treffer as $wert) {
                    ...
                    if ($CurrentTrans != '') {
                        ...
                        } // Array is compared to DB later for synchronization
                    }
                }
            } elseif ($file != '.' and $file != '..' and $file != '.svn' and is_dir($FilePath)) {
