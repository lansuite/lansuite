...
//---------------------------------------------------------+
    ...
    if (!$lgsl_feed_error)
    {
      if (preg_match("/_F2_(.*)_F2_/U", $http_reply, $match))
      {
        $server = unserialize(gzuncompress(base64_decode($match[1])));
      }
      elseif (preg_match("/_F1_(.*)_F1_/U", $http_reply, $match))
      {
        $server = unserialize(base64_decode($match[1]));
      }
