...
if (!$user_data['userid']) {
  ...
  // First name, last name, username, user ID
    ...
    if (IsAuthorizedAdmin()) {
        ($user_data['locked'])? $name .= ' '. $dsp->FetchIcon('index.php?mod=usrmgr&step=11&userid='. $_GET['userid'], 'locked', t('Account freigeben'))
        : $name .= ' '. $dsp->FetchIcon('index.php?mod=usrmgr&step=10&userid='. $_GET['userid'], 'unlocked', t('Account sperren'));
    }
    if (IsAuthorizedAdmin()) {
        $name .= ' '. $dsp->FetchIcon('index.php?mod=auth&action=switch_to&userid='. $_GET['userid'], 'assign', t('Benutzer wechseln'));
    }
    if ($_GET['userid'] == $auth['userid']) {
        $name .= ' '. $dsp->FetchIcon('index.php?mod=usrmgr&action=changepw', 'change_pw', t('Passwort ändern'));
    } elseif (IsAuthorizedAdmin()) {
        $name .= ' '. $dsp->FetchIcon('index.php?mod=usrmgr&action=newpwd&step=2&userid='. $_GET['userid'], 'change_pw', t('Passwort ändern'));
    }
    if (IsAuthorizedAdmin() or ($_GET['userid'] == $auth['userid'])) { # and $cfg['user_self_details_change']
        $name .= ' '. $dsp->FetchIcon('index.php?mod=usrmgr&action=change&step=1&userid='. $_GET['userid'], 'edit', t('Editieren'));
    }
    if ($auth['type'] >= 3) {
        $name .= ' '. $dsp->FetchIcon('index.php?mod=usrmgr&action=delete&step=2&userid='. $_GET['userid'], 'delete', t('Löschen'));
    }
    $name .= '</td></tr></table>';
  ...
  // Clan
    if ($cfg['signon_show_clan']) {
        ...
        if ($user_data['clan'] != '' and (IsAuthorizedAdmin() or ($user_data['clanid'] == $auth['clanid'] and $user_data['clanadmin'] == 1))) {
            $clan .= $dsp->FetchIcon('index.php?mod=clanmgr&action=clanmgr&step=10&clanid='. $user_data['clanid'], 'change_pw', t('Passwort ändern')) .
            $dsp->FetchIcon('index.php?mod=clanmgr&action=clanmgr&step=30&clanid='. $user_data['clanid'], 'edit', t('Editieren'));
        }
        $clan .= '</td></tr></table>';
  ...
  // Party Checkin, paid, ...
    if ($party->count > 0) {
        ...
        // Paid
        ($user_party['paid'])? $party_row .= ', '. $dsp->FetchIcon($link, 'paid', t('Bezahlt')) : $party_row .= ', '. $dsp->FetchIcon($link, 'not_paid', t('Nicht bezahlt'));
        ...
        if ($user_party['checkin']) {
            $party_row .= ' '. $dsp->FetchIcon($link, 'in', t('Eingecheckt')) .'['. $func->unixstamp2date($user_party['checkin'], 'datetime') .']';
        } else {
            $party_row .= ' '.$dsp->FetchIcon($link, 'not_in', t('Nicht eingecheckt'));
        }
        ...
        if ($user_party['checkout']) {
            $party_row .= ' '. $dsp->FetchIcon($link, 'out', t('Ausgecheckt')) .'['. $func->unixstamp2date($user_party['checkout'], 'datetime') .']';
        } else {
            $party_row .= ' '.$dsp->FetchIcon($link, 'not_out', t('Nicht ausgecheckt'));
        }

        if (IsAuthorizedAdmin() and $user_party['checkin'] > 0 and $user_party['checkout'] > 0) {
            $party_row .= $dsp->FetchIcon('index.php?mod=guestlist&step=22&userid='. $_GET['userid'], 'delete', 'Reset Checkin');
        }
  ...
  // Seating
    if ($func->isModActive('seating')) {
        ...
        if ($user_data_seating['block'] == '') {
            ...
            if (IsAuthorizedAdmin()) {
                $seat .= ' '. $dsp->FetchIcon("index.php?mod=seating&action=seatadmin&step=20&blockid={$user_data_seating['block']}&row={$user_data_seating['row']}&col={$user_data_seating['col']}&userid={$user_data['userid']}", 'delete', t('Löschen'));
            }
        }
        if (IsAuthorizedAdmin()) {
            $seat .= ' '. $dsp->FetchIcon('index.php?mod=seating&action=seatadmin&step=2&userid='. $_GET['userid'], 'edit', t('Editieren'));
        }
  ...
  //kontostand
    if ($func->isModActive('foodcenter')) {
        ...
        if ($result['total'] == "") {
          ...
          $kontostand .= ', '. t('Zahlung vornehmen') .': '.$dsp->FetchIcon('index.php?mod=foodcenter&action=account&act=payment&step=2&userid='.$_GET['userid'], 'paid');
  ...
  // Mail
    ...
    if ($auth['login'] and $func->isModActive('mail')) {
        $mail .= $dsp->FetchIcon('index.php?mod=mail&action=newmail&step=2&userID='. $_GET['userid'], 'send_mail', t('LANSuite-Mail an den User senden')) .' ';
    }
    $mail .= '</td></tr></table>';
  ...
  // Messenger
    ...
    if ($auth['login'] and $func->isModActive('msgsys')) {
        $messenger .= $dsp->FetchIcon('index.php?mod=msgsys&action=addbuddy&step=2&userid='. $_GET['userid'], 'add_user', t('Den User zu deiner Buddyliste hinzufügen')) .' ';
    }
