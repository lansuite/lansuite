...
if (!$cfg['sys_internet']) $func->information(t('Diese Funktion ist nur im Internetmodus verfÃ¼gbar')/* TRANS */, "");
	...
	case 2: // Email prüfen, Freischaltecode generieren, Email senden
		...
		if ($user_data['username'] == "LS_SYSTEM"){
			$func->information(t('FÃ¼r den System-Account darf kein neues Passwort generiert werden')/* TRANS */, "index.php?mod=usrmgr&action=pwrecover&step=1");
			...
			$func->confirmation(t('Ihnen wurde nun eine Freischalte-URL an die angegebene Emailadresse gesendet. Mit dem Aufruf dieser URL wird Ihr neues Passwort generiert werden.')/* TRANS */, "index.php");
	...
	default:
		...
		$dsp->AddSingleRow(t('Bitte geben Sie die Email-Adresse ein, mit der Sie sich am System angemeldet haben')/* TRANS */);
		$dsp->AddTextFieldRow("pwr_mail", t('Ihre Email')/* TRANS */, $pwr_mail, $mail_error);
