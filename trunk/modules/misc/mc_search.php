...
while($res = $db->fetch_array($row)) if($res['relatedto_item']) $list[$res['relatedto_item']] = $res['relatedto_item'];
$db->free_result($row);
...
while($res = $db->fetch_array($row)) if($res['relatedto_id']) $list[$res['relatedto_id']] = $res['relatedto_id'];
$db->free_result($row);
...
if ($auth['type'] >= 3) $ms2->AddMultiSelectAction(t('Löschen'), 'index.php?mod=misc&action=mc_search&step=10', 1);

$ms2->PrintSearch('index.php?mod=misc&action=mc_search', 'c.commentid');
?>
