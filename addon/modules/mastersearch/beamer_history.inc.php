...
$this->config['sql_statment']     = "SELECT m.mID, m.zeit, m.uID, m.bID, m.text, b.bezeichnung FROM {$config["tables"]["beamer_history"]} AS m LEFT JOIN {$config["tables"]["beamer_beamer"]} AS b ON b.bID = m.bID";
...
$this->config['inputs'][$z]['title']   = $lang['beamer']['search']['user'];
$this->config['inputs'][$z]['name']    = "search_select2";
$this->config['inputs'][$z]['type']    = "select";
$this->config['inputs'][$z]['sql'][1]     = "m.uID";
	$row = $db->query("SELECT * FROM {$config['tables']['user']} ORDER BY username");
	$user = array("all" => $lang['beamer']['search']['all']);
	while ($res = $db->fetch_array($row)){
		if(is_array($data)){
			$user[$res['userid']] = $res['username'];
		}
	}
$this->config['inputs'][$z]['options'] = $user;


// Der Titel der Suche.
$this->config['title']            = "User-MSG löschen:";
// ORDER BY - Teil des SQL-Aufrufes
$this->config['orderby']          = "m.zeit, DESC";
...
$this->config['result_fields'][$z]['maxchar']  = "20";
$z++;
$this->config['result_fields'][$z]['name']     = $lang['beamer']['search']['uhrzeit'];
$this->config['result_fields'][$z]['sqlrow']   = "m.zeit";
$this->config['result_fields'][$z]['row']      = "zeit";
