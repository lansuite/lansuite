...
/**
* Magic Debugfunction
* Calls addvar from Class debug. You can use it 2 Ways. Just pass a Variable
* or description+Variable. See Example. Caution : for set a Variablecaption
* like $variable you must User single quote. ('$variable')
* Example :
* <code>
* // Just an Variable. Debugoutput generate an automatic Name lika Debugvar_x
* d($config);
* // Or Name plus Variabe. Use single quote
* d('$config', $config);
* </code>
*
*/
function d() {
    ...
    if ($debug->mode > 0) {
        if ($arg_count==2) {
            $debug->addvar($arg_vars[0], $arg_vars[1]);
        } elseif ($arg_count==1) {
   ...
   */
    function tracker($event) {
        if ($this->mode > 0) {
            ...
            if(!$this->timer_first OR $event === false) {
                $this->timer_first = $time;
            }
            if(!$this->timer_last OR $event === false) {
    ...
    /**
     * Print Timer (HTML)
     * @return string HTML-Output Timer
     * @deprecated
     */
    function timer_show() {
        if ($this->mode > 0) {
    ...
    /**
     * Print overall time for Timer
     * @return string HTML-Output overall time
     * @todo Not exact, generate on Call
     */
    function timer_all() {
        if ($this->mode > 0) {
